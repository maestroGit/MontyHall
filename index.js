...
const manteinselection = () => {
    ...
    mantein++;
    changedoor = false;
    win(doorsel, doorwin, changedoor);
};

const changeselection = () => {
    changedoor = true;
    console.log("change selection", { changedoor });
    // quitar lisener
    btnswitch.removeEventListener("click", manteinselection);
    // eleminar visibility grid o btn
    ...
    if (doorchoosed[0] == doorprize[0]) {
        ...
        changes++;
        winersChanges++;
        win(changedoorsel, doorwin, changedoor);
        drawcar(doorprize[0]);
    }
};

// Check for a match
const win = (selectdoor, prizedoor, changedoor) => {
    console.log(selectdoor, { changedoor });
...
const start = () => {
    ...
    for (let i = 0; i < doors.length; i++) {
        ...
        //doors[i].innerHTML("<div class handler></div>")
